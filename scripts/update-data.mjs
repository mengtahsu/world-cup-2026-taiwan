...
async function getVideos(){try{const xml=await fetch(YOUTUBE).then(r=>{if(!r.ok)throw Error(r.status);return r.text()});return [...xml.matchAll(/<entry>([\s\S]*?)<\/entry>/g)].slice(0,6).map(x=>{const s=x[1],id=clean(s.match(/<yt:videoId>(.*?)<\/yt:videoId>/)?.[1]),title=clean(s.match(/<title>([\s\S]*?)<\/title>/)?.[1]);return{id,title,label:'FIFA 官方影片',url:`https://www.youtube.com/watch?v=${id}`}}).filter(x=>x.id).slice(0,3)}catch(e){console.warn('Videos unavailable:',e.message);return[]}}
